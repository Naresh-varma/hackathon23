...
const main = () => new Promise((resolve, reject) => {
    const indexDetails = mapper['knowledge'];
    BluebirdPromise.mapSeries(knowledgeBaseData, (data) => new Promise((resolve, reject) => {
        let vectorText = '';
        const fields = indexDetails.fields;
        _.each(fields, (field) => {
          if (data[field]) vectorText += `${data[field]} `;
        });
        if (!vectorText) return reject('No text found to embed');
        getEmbeddings(vectorText)
            .then((embedRes) => {
                ...
                if (embedRes) {
                    ...
                    } else data[`${indexDetails.vectorField}`] = embedRes?.data[0]?.embedding;
                }
                return resolve();
            });
    }))
      .then(() => {
        console.log('completed');
          return resolve(makeBulkRequestToEls(knowledgeBaseData, indexDetails.index));
      })
